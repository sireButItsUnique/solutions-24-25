    ...
    for (int i = 1; i <= n; ++i) {
        if (visited[i] == -1) {  // If this house is unprocessed
            ...
            if (visited[node] == 1) {
                ...
                // Found a cycle
                int cycle_start = 0;
                while (current_path[cycle_start] != node) {
                    ++cycle_start;
                }
                path_length = current_path.size() - cycle_start;

                // Mark all nodes in the cycle
                for (int j = cycle_start; j < current_path.size(); ++j) {
                    result[current_path[j]] = path_length;
                }
            }

            // Process nodes before the cycle
            for (int j = current_path.size() - 1; j >= 0; --j) {
                if (result[current_path[j]] == 0) {
                    ++path_length;
                    result[current_path[j]] = path_length;
                }
                visited[current_path[j]] = 1;  // Mark as fully processed
            }
        }
    }
